let vm = new Vue({
    ...
    methods:{
        selectPage:function (pageNum,pageSize) {
            axios({
                url:`manager/area/selectPage/${pageNum}/${pageSize}`,  //任然受到  base标签影响
                params:this.params
        ...
        toUpdate:function (id) {
            ...
            layer.open({
                ...
                content: 'manager/area/toUpdate', //iframe的url
                end: () => {
                    if(layer.message!=''){
        ...
        toDetail:function (id) {
            ...
            layer.open({
                ...
                title: false,//子窗口标题    false则不显示
                area: ['80%', '80%'],        //窗口大小  [宽,高]   支持px和占据父窗口比例
                content: 'manager/area/toDetail', //iframe的url
                end: () => {
                    if(layer.message!=''){
                        this.selectPage(1,5);
                    }
 ...
 * 1.change事件绑定
 2.获取事件源，得到File对象
 3.封装FormData对象，
 4.将File对象追加到FormData对象中
 设置key与后台接收方法参数对应
 5.ajax上传处理，设置content-type为multipart/form-data,设置提交方式为post
 */
